...
YTrainRaw = nacitajMNISTStitky(trainLabelsFile);

XTest = nacitajMNISTObrazky(testImagesFile);
YTestRaw = nacitajMNISTStitky(testLabelsFile);

% ===============================
% Prevod stitkov na categorical
% ===============================
classNames = string(0:9);
YTrain = categorical(YTrainRaw, 0:9, classNames);
YTest  = categorical(YTestRaw,  0:9, classNames);
...
% ===============================
% Architektura MLP bez konvolucii
% ===============================
layers = [
...
if ~isempty(best_info)
    ...
    xlabel('Iteration');
    ylabel('Loss');
    title('Najlepsi MLP beh: Training Loss');
...
figure('Name', 'Digit Samples', 'Position', [100, 100, 1000, 400]);
...
for digit = 0:9
    ...
    if isempty(idx)
        warning('Digit %d not found!', digit);
        continue;
    end
...
figure('Name', 'Final Confusion Matrix for Digit Testing');
confusionchart(all_true, all_pred);
title('Confusion Matrix for Digit Testing (0-9)');

fprintf('\n== Done ==\n');

% ===============================
% Pomocna funkcia na nacitanie obrazkov MNIST z IDX suboru
% Vystup ma tvar 28 x 28 x 1 x N
% ===============================
